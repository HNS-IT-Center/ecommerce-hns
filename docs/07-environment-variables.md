...
### 2.7 OPSIONAL — Auth (Jika Bikin Session Sendiri)

Kalau customer login via WooCommerce sudah cukup, ini tidak perlu. Kalau bangun session Next.js sendiri (NextAuth / Auth.js):

| Variable | Deskripsi |
|---|---|
| `AUTH_SECRET` | Random 32+ karakter untuk sign JWT/session |
| `AUTH_URL` | URL canonical untuk callback |
...
4. Jalankan `pnpm dev`. Jika ada error `EnvSchema.parse(...)` — env belum lengkap.
...
Jalankan `pnpm env:check` untuk validasi tanpa harus start server.
