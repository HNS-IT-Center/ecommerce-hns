...
- Upload **foto** produk lewat **Cloudflare R2** — satu-satunya jalur, tidak ada fallback. Alurnya: `admin/produk/image-uploader.tsx` → `POST /api/admin/media` → `uploadMedia()` di `lib/api/cloudflare/r2.ts`. Butuh 5 env (`R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET_NAME`, `NEXT_PUBLIC_R2_PUBLIC_URL`) — lihat `docs/07-environment-variables.md` §2.10. URL hasil upload disimpan di tabel `product_images` (Prisma).
- **WordPress Media REST API sudah TIDAK dipakai** untuk foto produk. `lib/api/wordpress/media.ts` adalah kode mati (nol importer) dan mengekspor fungsi bernama **`uploadMedia`** — nama yang identik dengan milik R2. Jangan terima saran auto-import dari berkas itu; yang benar `@/lib/api/cloudflare/r2`. Berkasnya menunggu dihapus.
