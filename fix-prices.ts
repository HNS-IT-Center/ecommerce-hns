...
async function fixVariablePrices() {
  ...
  for (const p of variables) {
    if (p.variations && p.variations.length > 0) {
      const minRegular = Math.min(...p.variations.map((v: any) => v.regularPrice ? Number(v.regularPrice) : Infinity).filter((v: any) => v !== Infinity));
      const minSale = Math.min(...p.variations.map((v: any) => v.salePrice ? Number(v.salePrice) : Infinity).filter((v: any) => v !== Infinity));
...
fixVariablePrices().catch(console.error).finally(() => process.exit(0));
